...
END FUNCTION --}}}
--------------------------------------------------------------------------------
FUNCTION gl2_setActions(l_row,l_max,l_allowedActions)
...
#+
#+ @param l_what Text for what you want to confirm
#+ @returns TRUE/FALSE
FUNCTION gl2_confirm(l_what)
	DEFINE l_what STRING

	IF gl_winQuestion(%"Confirm?",SFMT(%"Comfirm %1",l_what),	"N",%"Yes|No","exclamation") = "Yes" THEN
		RETURN TRUE
	END IF
	RETURN FALSE
...
FUNCTION gl2_checkRec(l_ex, l_key, l_sql)
	DEFINE l_ex SMALLINT
	DEFINE l_key STRING
	DEFINE l_sql STRING
	...
	IF l_key IS NULL OR l_key = " " OR l_key.getLength() < 1 THEN
		CALL fgl_winMessage(%"Warning",%"You entered a NULL Key value!","exclamation")
		RETURN FALSE
	END IF

	PREPARE gl2_checkrec_pre FROM l_sql
	DECLARE gl2_checkrec_cur CURSOR FOR gl2_checkrec_pre
	OPEN gl2_checkrec_cur
	LET l_exists = TRUE
	FETCH gl2_checkrec_cur
	IF STATUS = NOTFOUND THEN LET l_exists = FALSE END IF
	CLOSE gl2_checkrec_cur
	IF NOT l_exists THEN
		IF l_ex THEN
			CALL fgl_winMessage(%"Warning",%"Record '"||l_key||"' doesn't Exist!","exclamation")
			RETURN FALSE
		END IF
	ELSE
		IF NOT l_ex THEN
			CALL fgl_winMessage(%"Warning",%"Record '"||l_key||"' already Exists!","exclamation")
			RETURN FALSE
		END IF
	END IF
	RETURN TRUE
