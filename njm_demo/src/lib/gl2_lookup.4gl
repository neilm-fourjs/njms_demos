...
#+ $Id: gl2_lookup.4gl 688 2011-05-31 09:58:14Z  $
...
-- If only one record then just return it's key.
	IF tot_recs = 1 THEN
		...
		OPEN listcur
		FETCH listcur INTO m_data_ar[1].cf1,
											m_data_ar[1].cf2,
											m_data_ar[1].cf3,
											m_data_ar[1].cf4,
											m_data_ar[1].cf5,
		...
		RETURN m_data_ar[1].cf1 --,data_ar[1].cf2
	END IF
...
-- else use a paged array
	IF tot_recs < MAX_RECS THEN
		...
		FOREACH listcur INTO INTO_DATA_AR( recs )
			CALL gl_lookup_mvdta(m_data_ar,recs,ctyp)
...
--		CALL ui.interface.refresh()
		...
		DISPLAY ARRAY m_data_ar TO tablistv.* ATTRIBUTE(COUNT=recs)
			BEFORE ROW
